  ...
  return (
    ...
    >
      <div className="flex flex-col h-full p-6 sm:p-8 md:p-10">
        ...
        <h3 className="font-serif text-2xl sm:text-3xl tracking-wide text-charcoal">
          {pkg.name}
        </h3>
        <p className="mt-2 font-sans text-sm tracking-wider text-charcoal-light">
          {pkg.description}
        </p>

        <div className="h-px my-5 sm:my-6 bg-charcoal/10" />

        <ul className="space-y-3 flex-1">
          {pkg.includes.map((item) => (
            <li
              key={item}
              className="flex items-start gap-3 font-sans text-[13px] sm:text-sm tracking-wider text-charcoal-light leading-relaxed"
            >
              <span className="mt-0.5 text-gold text-sm shrink-0">✦</span>
              {item}
            </li>
          ))}
        </ul>
        ...
        {pkg.perfectFor && (
          <p className="mt-6 font-serif text-base italic text-charcoal-light">
            {pkg.perfectFor}
          </p>
        )}
          ...
          >
            {premiumAddOns.map((addon) => (
              <StaggerItem key={addon.name}>
                <div className="flex flex-col h-full p-6 sm:p-8 md:p-10 border border-cream-dark bg-white hover:border-gold/50 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  <h4 className="font-serif text-2xl sm:text-3xl tracking-wide text-charcoal">
                    {addon.name}
                  </h4>
                  <div className="h-px bg-cream-dark my-5 sm:my-6" />
                  <p className="font-sans text-sm tracking-wider text-charcoal-light leading-relaxed">
                    {addon.description}
                  </p>
                </div>
              </StaggerItem>
